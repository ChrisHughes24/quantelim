...
def ite (p : Poly n) : ∀ (_φ _ψ : QuantFreeFormula n), QuantFreeFormula n
  | tru, tru => tru
  | fals, fals => fals
  | φ, ψ => ite' p φ ψ

@[simp]
theorem eval_ite (p : Poly n) φ ψ : (ite p φ ψ).eval =
    ({x | p.eval x = 0} ∩ φ.eval) ∪ ({x | p.eval x ≠ 0} ∩ ψ.eval) := by
  cases φ <;> cases ψ<;> simp [eval, Set.ext_iff]; tauto
