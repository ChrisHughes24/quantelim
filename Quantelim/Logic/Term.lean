...
noncomputable def eval : {n : ℕ} → Term n → (vars : Fin n → K) → K
  | _, ofInt q, _ => Int.cast q
  | _, var i, vars => vars i
  | _,  add t₁ t₂, vars => eval t₁ vars + eval t₂ vars
  | _, mul t₁ t₂, vars => eval t₁ vars * eval t₂ vars
  | _, neg t, vars => -eval t vars
  | _, sub t₁ t₂, vars => eval t₁ vars - eval t₂ vars
